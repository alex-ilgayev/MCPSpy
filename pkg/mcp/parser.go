...
func (p *Parser) ParseDataStdio(e event.Event) {
	...
	for {
		...
		if err := p.validateMCPMessage(jsonRpcMsg); err != nil {
			logrus.WithFields(jsonRpcMsg.LogFields()).WithError(err).Debug("Invalid MCP message")
			return
		}
...
func (p *Parser) ParseDataHttp(e event.Event) {
	...
	for {
		...
		if err := p.validateMCPMessage(jsonRpcMsg); err != nil {
			logrus.WithFields(jsonRpcMsg.LogFields()).WithFields(eventFields).WithError(err).Debug("Invalid MCP message")
			return
		}
